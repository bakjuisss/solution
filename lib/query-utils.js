...
function filterResultsForContext(results, query, { maxCount = 4, keywordOnly = false } = {}) {
  ...
  const minScore = keywordOnly
    ? Math.max(topScore * 0.4, 0.15)
    : Math.max(topScore * 0.55, 0.35);
